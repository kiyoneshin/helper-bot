...
class RenameModal(discord.ui.Modal, title="Đổi tên phòng"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"✅ Đã đổi tên phòng: **{new_name}**", ephemeral=True)
...
class LimitModal(discord.ui.Modal, title="Giới hạn người dùng"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ Đã đặt giới hạn: {label}", ephemeral=True)
        except ValueError:
...
class PermitModal(discord.ui.Modal, title="Cho phép người dùng"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not match:
            await interaction.response.send_message("❌ Không tìm thấy ID hợp lệ!", ephemeral=True)
            return
        if not isinstance(interaction.guild, discord.Guild):
            return
        target = interaction.guild.get_member(int(match.group()))
        if not target:
            await interaction.response.send_message("❌ Không tìm thấy thành viên trong server!", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"✅ Đã cấp quyền vào phòng cho **{target.display_name}**!", ephemeral=True)
...
class RejectSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if target.id == interaction.user.id or target.bot:
            await interaction.response.send_message("❌ Không thể chọn người này!", ephemeral=True)
            return
...
class TransferSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if new_owner.bot or new_owner.id == interaction.user.id:
            return await interaction.response.send_message("❌ Không thể chọn người này!", ephemeral=True)
        if new_owner not in self.channel.members:
            return await interaction.response.send_message("❌ Người này không có trong phòng!", ephemeral=True)
        pool = getattr(self.bot, "db_pool", None)
        if pool:
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if pool:
            ...
            if row and interaction.user.id != row["owner_id"]:
                return await interaction.response.send_message("❌ Chỉ **chủ phòng** mới có thể thay đổi!", ephemeral=True)
        ...
        if not has_perm:
            return await interaction.response.send_message("❌ Role của bạn chưa có quyền này!", ephemeral=True)
...
class PermissionsSelect(discord.ui.Select):
    def __init__(self, channel: discord.VoiceChannel, bot: commands.Bot, owner_id: int):
        ...
        super().__init__(
            ...
            options=[
                ...
                discord.SelectOption(label="Cho phép", description="Cấp quyền vào cho 1 người",            emoji="✅",    value="permit"),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if pool:
            row = await _get_active_channel(pool, self.channel.id)
            if row and interaction.user.id != row["owner_id"]:
                return await interaction.response.send_message("❌ Chỉ **chủ phòng** mới có thể đổi quyền!", ephemeral=True)
        if not isinstance(interaction.guild, discord.Guild) or not isinstance(interaction.user, discord.Member):
            return

        if val in ("lock", "unlock"):
            if not await _check_perm(pool, interaction.guild.id, interaction.user, "can_lock") if pool else False:
                return await interaction.response.send_message("❌ Chưa có quyền khóa/mở phòng!", ephemeral=True)
            ...
            if not await _check_perm(pool, interaction.guild.id, interaction.user, "can_hide") if pool else False:
                return await interaction.response.send_message("❌ Chưa có quyền ẩn/hiện phòng!", ephemeral=True)
            ow = self.channel.overwrites_for(interaction.guild.default_role)
            ...
            if not await _check_perm(pool, interaction.guild.id, interaction.user, "can_transfer") if pool else False:
                return await interaction.response.send_message("❌ Chưa có quyền chuyển chủ phòng!", ephemeral=True)
            if await _check_perm(pool, interaction.guild.id, interaction.user, "is_persistent") if pool else False:
                return await interaction.response.send_message("❌ Kênh cá nhân (Level 50+) không thể chuyển nhượng!", ephemeral=True)
...
class VoiceControlView(discord.ui.View):
    ...
    async def btn_claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not pool or not isinstance(interaction.guild, discord.Guild) or not isinstance(interaction.user, discord.Member):
            return await interaction.response.send_message("❌ Lỗi hệ thống!", ephemeral=True)

        row = await _get_active_channel(pool, self.channel.id)
        if not row:
            return await interaction.response.send_message("❌ Không tìm thấy thông tin phòng!", ephemeral=True)

        claimer = interaction.user
        current_owner_id = row["owner_id"]

        if claimer.id == current_owner_id:
            return await interaction.response.send_message("❌ Bạn đang là chủ phòng rồi!", ephemeral=True)
        if claimer not in self.channel.members:
            return await interaction.response.send_message("❌ Bạn phải ở trong phòng để nhận quyền!", ephemeral=True)
            
        old_owner = self.channel.guild.get_member(current_owner_id)
        if old_owner:
            if old_owner in self.channel.members:
                return await interaction.response.send_message(
                    f"❌ **{old_owner.display_name}** vẫn trong phòng! Không thể Claim.", ephemeral=True
                )
            if await _check_perm(pool, interaction.guild.id, old_owner, "is_persistent"):
                return await interaction.response.send_message(
                    "❌ Đây là Kênh Cá Nhân của người khác. Không thể Claim!", ephemeral=True
                )
    ...
    async def btn_delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if pool:
            row = await _get_active_channel(pool, self.channel.id)
            if row and interaction.user.id != row["owner_id"]:
                return await interaction.response.send_message("❌ Chỉ **chủ phòng** mới có quyền xóa kênh thủ công!", ephemeral=True)
        
        await interaction.response.send_message("✅ Đang xóa kênh...", ephemeral=True)
        try:
