...
class ArcaneLevelSync(commands.Cog):
    ...
    async def synclv_cmd(self, ctx: commands.Context):
        ...
        if not isinstance(channel, (discord.TextChannel, discord.Thread, discord.VoiceChannel)):
            await ctx.send("❌ Kênh cấu hình CHANNEL_ID không hợp lệ hoặc bot không truy cập được.")
            return
        ...
        # Regex bóc tách số từ chuỗi 'đã lên level *<số>*'
        level_regex = re.compile(r'đã lên level \*(\d+)\*', re.IGNORECASE)
