...
class GiveawaySession:
    ...
    async def _ask_text(self, prompt: str) -> str:
        ...
        while True:
            try:
                ...
                if self.prompt_msg:
                    await self.prompt_msg.edit(content="<:symbol_wrong:1536629915598848072> Đã hủy do quá thời gian (60s).", view=None)
                if self.msg:
    ...
    async def ask_1_ga(self):
        while True:
            ...
            if t > 0:
                ...
                self._update_fields()
                break
            msg_err = await self.ctx.send("<:symbol_wrong:1536629915598848072> Thời gian không hợp lệ. Hãy thử lại (VD: 1m).")
        ...
        while True:
            ...
            if len(parts) >= 2:
                ...
                if t > 0 and parts[1].isdigit() and int(parts[1]) > 0:
                    ...
                    self._update_fields()
                    break
            msg_err = await self.ctx.send("<:symbol_wrong:1536629915598848072> Không hợp lệ. Hãy thử lại (VD: `1m 10`).")
            self.bot.loop.create_task(msg_err.delete(delay=3))
    ...
    async def ask_2(self):
        while True:
            ...
            if ans.isdigit() and int(ans) > 0:
                ...
                self._update_fields()
                break
            msg_err = await self.ctx.send("<:symbol_wrong:1536629915598848072> Vui lòng nhập một số nguyên dương.")
            self.bot.loop.create_task(msg_err.delete(delay=3))
        ...
        while True:
            ...
            if m:
                ...
                if role:
                    ...
                    self._update_fields()
                    break
            msg_err = await self.ctx.send("<:symbol_wrong:1536629915598848072> Role không hợp lệ. Vui lòng thử lại.")
            self.bot.loop.create_task(msg_err.delete(delay=3))
    ...
    async def show_confirm(self):
        view = ConfirmView()
        if self.prompt_msg:
            await self.prompt_msg.edit(content="<:symbol_right:1536629912515903578> **Cấu hình hoàn tất!** Vui lòng kiểm tra lại thông tin và xác nhận.", view=view)
        res = await view.wait()
        ...
        if view.action == "confirm":
            if not self.channel_id:
                msg_err = await self.ctx.send("<:symbol_wrong:1536629915598848072> Bạn chưa chọn kênh hợp lệ. Vui lòng sửa lại Kênh.")
            ...
            if self.prompt_msg:
                await self.prompt_msg.edit(content="<:symbol_right:1536629912515903578> Đã lên lịch Giveaway thành công!", view=None)
...
class GiveawayCog(commands.Cog):
        ...
        if is_ga:
            ...
            if msg:
                ...
                try:
                    ...
                    await user.send(f"<:symbol_wrong:1536629915598848072> Bạn đã bị cấm tham gia Giveaway nên không thể thả reaction.\n**Lý do:** {reason}")
                except:
                    pass
    ...
    async def gaban_cmd(self, ctx: commands.Context, user: discord.User, *, reason: str = "Không có lý do"):
        if user.bot or user.id == ctx.author.id:
            await ctx.send("<:symbol_wrong:1536629915598848072> Bạn không thể ban bot hoặc chính mình!")
            return
        ...
        await ctx.send(f"<:symbol_right:1536629912515903578> Đã thêm **{user.display_name}** vào danh sách đen Giveaway!\nLý do: {reason}")
    ...
    async def gabanlist_cmd(self, ctx: commands.Context):
        ...
        if not records:
            await ctx.send("<:symbol_right:1536629912515903578> Hiện tại không có ai bị cấm tham gia Giveaway.")
            return
    ...
    async def gareroll_cmd(self, ctx: commands.Context, message_id: str, winners_count: int = 1):
        if not message_id.isdigit():
            await ctx.send("<:symbol_wrong:1536629915598848072> ID tin nhắn không hợp lệ.")
            return
        ...
        try:
            ...
            await ctx.send("<:symbol_wrong:1536629915598848072> Không tìm thấy tin nhắn trong kênh này. Hãy dùng lệnh ở cùng kênh với Giveaway đó.")
            return
        ...
        reaction = discord.utils.get(msg.reactions, emoji=GA_EMOJI)
        if not reaction:
            await ctx.send("<:symbol_wrong:1536629915598848072> Không có ai tham gia Giveaway này.")
            return
