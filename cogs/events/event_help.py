...
def build_eco_embed() -> discord.Embed:
    embed = discord.Embed(
        title="🏕️ Khu Sinh Thái (Farm, Mine, Fish)",
        description="Tham gia các hoạt động sinh thái để thu thập tài nguyên, nông sản, khoáng sản và cá! Tất cả sẽ được cất vào Túi đồ.",
        color=COLOR_THEME
    )
    embed.add_field(
        name="🚜 y!farm (hoặc y!nongtrai)",
        value="Mở Nông Trại 3x3. Trồng cây, tưới nước, và tận dụng cơ chế Cộng Hưởng & Cây Khổng Lồ để kiếm lời lớn.",
        inline=False
    )
    embed.add_field(
        name="🚜 y!upgrade (hoặc y!nangcap)",
        value="Dùng điểm sự kiện để mua thêm ô đất cho Nông Trại (Tối đa 9 ô).",
        inline=False
    )
    embed.add_field(
        name="🛒 y!farmshop (hoặc y!muahat)",
        value="Cửa hàng mua sỉ/lẻ các loại hạt giống nông nghiệp.",
        inline=False
    )
    embed.add_field(
        name="⛏️ y!mine (hoặc y!dao, y!khoamo)",
        value="Vào Khu Mỏ đập đá kiếm quặng (Tiêu hao 10 Thể Lực).",
        inline=False
    )
    embed.add_field(
        name="🎣 y!fish (hoặc y!cauca)",
        value="Đến Hồ Câu thử tài phản xạ bắt cá (Tiêu hao 15 Thể Lực).",
        inline=False
    )
    embed.add_field(
        name="🎒 y!bag (hoặc y!khodo)",
        value="Túi đồ sinh thái. Chứa Nông sản, Hạt giống, Quặng, Cá. Bán đồ để lấy điểm sự kiện.",
        inline=False
    )
    embed.add_field(
        name="🏭 y!machine",
        value="Khu vực Chế biến tài nguyên thành hàng nghệ nhân (Đang phát triển).",
        inline=False
    )
    return embed
...
class EventHelpSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(
                label="Khu Sinh Thái (Farm/Mine/Fish)",
                value="eco",
                emoji="🏕️",
                description="Trồng trọt, câu cá, đào mỏ và túi đồ"
            ),
            discord.SelectOption(
