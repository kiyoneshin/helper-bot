...
EVENT_MILESTONES = {
    ...
    40000: {"name": "Tinh Anh Server", "points": 0, "items": {"fake_ban_card": 1}, "role_id": ROLE_DOI_CAO_PHIM, "tickets": 0},
    60000: {"name": "Bàn Tay Đen Tối", "points": 3000, "items": {"thief_card": 1, "ghost_ping_card": 1}, "tickets": 0},
    75000: {"name": "Tuyệt Đỉnh F2P", "points": 10000, "items": {}, "role_id": ROLE_HUYEN_THOAI, "tickets": 0},
    100000: {"name": "Chúa Tể Sự Kiện", "points": 0, "items": {"jail_card": 2}, "tickets": 0},
}
...
class MilestoneCog(commands.Cog):
    ...
    async def claim_cmd(self, ctx: commands.Context) -> None:
        ...
        for moc_diem, data in valid_milestones:
            ...
            if data.get("role_id"):
                roles_to_add.append(data["role_id"])
        ...
        if getattr(ctx, "guild", None):
            for role_id in roles_to_add:
                try:
                    ...
                    log.error(f"Lỗi khi cấp role: {e}")
