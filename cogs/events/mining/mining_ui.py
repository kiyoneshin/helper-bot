...
from .mining_config import (
    ...
    get_mining_loot, get_mining_display_weights, get_mining_effective_weights
)
...
def build_mining_embed(
    ...
    # Tính stamina_cost thực tế có tính buff
    effective_stamina_cost = STAMINA_PER_HIT
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"Hãy đập đá để tìm quặng quý. Mỗi lần đập tốn **{effective_stamina_cost}** thể lực.\n"
        ),
        color=0x7f8c8d,
    )
    ...
    # Hiển thị tỉ lệ thực tế (có buff) nếu có dữ liệu, ngược lại dùng bảng gốc
    if boosts is not None or skills_data is not None:
        display_weights = get_mining_effective_weights(pickaxe_level, boosts or {}, skills_data or {})
        label = "<:symbol_mineral:1536007704076615792> Tỉ Lệ Rớt Đồ (Thực Tế)"
    else:
        display_weights = get_mining_display_weights(pickaxe_level)
        label = "<:symbol_mineral:1536007704076615792> Tỉ Lệ Rớt Đồ (Base)"
...
class MiningView(discord.ui.View):
    ...
    async def mine_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if lb_tier and _has_profession(skills_data, "mining", "excavator"):
            # 30% cơ hội nâng 1 tier, 10% nâng 2 tier, với multiplier x1.1 - x1.5
            roll = _random.random()
            if roll < 0.10 and lb_tier < 66:   # x1.5 chance: nâng 2 tier
                lb_tier = min(lb_tier + 2, 66)
            elif roll < 0.40 and lb_tier < 66:  # x1.1-1.5 chance: nâng 1 tier
                lb_tier = min(lb_tier + 1, 66)
