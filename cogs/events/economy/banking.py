...
class BankingCog(commands.Cog):
    ...
    async def vayno_cmd(self, ctx: commands.Context, so_tien: str):
        ...
        if available_loan <= 0:
            await ctx.send(f"<:symbol_wrong:1536629915598848072> {ctx.author.mention} Bạn đã hết hạn mức vay! (Hạn mức: **{max_loan:,.0f}**, Đang nợ: **{current_debt:,.0f}**)")
            return
    ...
    async def trano_cmd(self, ctx: commands.Context, so_tien: str):
        ...
        if current_debt <= 0:
            await ctx.send(f"<:symbol_right:1536629912515903578> {ctx.author.mention} Bạn không có khoản nợ nào để trả!")
            return
            
        if current_points <= 0:
            await ctx.send(f"<:symbol_wrong:1536629915598848072> {ctx.author.mention} Bạn không có tiền trong ví để trả nợ!")
            return
        ...
        if amount > current_points:
            await ctx.send(f"<:symbol_wrong:1536629915598848072> {ctx.author.mention} Số dư ví không đủ! (Ví đang có: **{current_points:,.0f}**)")
            return
