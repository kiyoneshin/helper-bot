...
class BetModal(discord.ui.Modal, title="💰 Đặt Cược - Quả Bóng Tham Lam"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if not self.lobby_view.is_active:
            await interaction.response.send_message(
                "<:symbol_wrong:1536629915598848072> Sảnh đã đóng, hết chỗ chen chân rồi!",
                ephemeral=True,
            )
            return
        ...
        if err or bet is None:
            await interaction.response.send_message(
                f"<:symbol_wrong:1536629915598848072> {interaction.user.mention} {err}",
                ephemeral=True,
            )
            return
        ...
        if not ok:
            await interaction.response.send_message(
                f"<:symbol_wrong:1536629915598848072> {interaction.user.mention} Trừ tiền thất bại - chắc do số dư không đủ. Kiểm tra lại túi đi!",
                ephemeral=True,
            )
            return
...
class CrashLobbyView(discord.ui.View):
    ...
    async def bet_button(
        ...
        if lobby_msg is None:
            await interaction.response.send_message("<:symbol_wrong:1536629915598848072> Lỗi nội bộ sòng bài!", ephemeral=True)
            return
    ...
    async def cashout_button(
        ...
        async with self._lock:
            ...
            if user_id not in self.players_bets:
                await interaction.response.send_message(
                    "<:symbol_wrong:1536629915598848072> Đã đứng ngó mà còn bấm bậy, có cược đâu mà đòi chốt!",
                    ephemeral=True,
                )
                return
...
class CrashGame(commands.Cog):
    ...
    async def crash_cmd_error(self, ctx: commands.Context, error: Exception):
        if isinstance(error, (commands.MissingRequiredArgument, commands.BadArgument)):
            await ctx.send(f"<:symbol_wrong:1536629915598848072> {ctx.author.mention} Tính lên tàu bay dạo không vé hả? Cú pháp: `{ctx.prefix}crash <tiền_cược>`. Để biết thêm chi tiết hãy xài lệnh `{ctx.prefix}ehelp crash`")
        elif isinstance(error, commands.CommandInvokeError):
            log.error("Loi crash_cmd: %s", error.original, exc_info=True)
            await ctx.send(
                "<:symbol_wrong:1536629915598848072> Đã xảy ra lỗi nội bộ làm sập sòng Crash. "
